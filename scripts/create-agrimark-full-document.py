...
PARTNER_LOGO = ROOT / "public" / "agrimark-logo.jpg"
OUT = ROOT / "output" / "pdf" / "agrimark-commercial-collaboration-framework.pdf"
...
def page_important_note(c, page_no):
    y = start_page(c, "00", "Document Status", "This paper supports commercial and operational alignment between Smart Steel and Agrimark.", page_no, title_size=27)
    ...
    cards3(c, y - 24, [
        ("Audience", "Agrimark stakeholders", "Partnership, commercial, finance and operations teams."),
...
def page_exec(c, page_no):
    ...
    y = para(c, "Smart Steel and Agrimark have a potential opportunity to create a practical infrastructure collaboration for agricultural, commercial and energy-related customers.", MARGIN_X, y, CONTENT_W * 0.92, size=10.0, leading=12.4)
    y = para(c, "The proposed model combines Agrimark's customer relationships, finance capability, branch network and market credibility with Smart Steel's modular steel infrastructure systems, structural engineering capability, manufacturing base, technical support and Atlas digital platform.", MARGIN_X, y - 10, CONTENT_W * 0.92, size=10.0, leading=12.4)
    y = para(c, "The aim is to create a simple commercial path for Agrimark: generate customer opportunity and additional return, without carrying unnecessary technical, installation or operational complexity.", MARGIN_X, y - 10, CONTENT_W * 0.92, size=10.0, leading=12.4)
    y = heading(c, "The intended customer experience is simple:", MARGIN_X, y - 14, size=11)
    cards3(c, y, [
        ("Agrimark", "Customer channel", "Primary commercial relationship, customer-facing partner and finance channel."),
...
def page_rationale(c, page_no):
    y = start_page(c, "02", "Collaboration Rationale", "The proposed collaboration is based on a clear fit between Agrimark's market access and Smart Steel's infrastructure capability.", page_no)
    y = two_columns(c, y, "Agrimark brings", [
    ...
    note(c, y - 18, "Shared opportunity", "A customer can engage through Agrimark, access finance, select a structure, receive Smart Steel technical support and move through a controlled process from enquiry to delivery, installation and warranty support.", h=62)
    c.showPage()


def page_model(c, page_no):
    y = start_page(c, "03", "Proposed Collaboration Model", "The preferred structure is a protected dealer-price model, with Agrimark remaining the customer-facing commercial party.", page_no)
    y = flow(c, y - 8, [
        ("01", "Customer", "Agrimark qualifies the opportunity."),
        ("02", "Atlas", "Project information is captured."),
        ("03", "Review", "Smart Steel reviews scope and assumptions."),
        ("04", "Dealer quote", "Smart Steel quotes Agrimark."),
        ("05", "Proposal", "Agrimark issues customer proposal."),
        ("06", "Finance", "Payment or finance is managed."),
        ("07", "PO", "Agrimark instructs Smart Steel."),
        ("08", "Delivery", "Smart Steel executes agreed scope."),
    ], cols=4)
    y = heading(c, "Why this model", MARGIN_X, y - 18)
    y = bullet_list(c, [
        "Agrimark remains the primary customer relationship owner.",
...
def page_customer_journey(c, page_no):
    ...
    steps = [
        ("01", "Opportunity", "Agrimark receives or identifies a customer opportunity."),
        ...
        ("04", "Dealer quotation", "Smart Steel issues approved quote to Agrimark."),
        ("05", "Customer proposal", "Agrimark issues proposal and manages finance."),
        ("06", "Acceptance", "Customer accepts and Agrimark issues instruction."),
...
def page_commercial(c, page_no):
    ...
    y = cards3(c, y - 4, [
        ...
        ("Pilot return", "5% proposed", "Agrimark receives a proposed return on qualifying structural supply value."),
...
def page_return(c, page_no):
    y = start_page(c, "06", "Pilot Return Basis", "The proposed starting commercial return for Agrimark is 5% during the pilot and initial launch period.", page_no)
    ...
    bullet_list(c, [
        "The opportunity is properly registered through the Agrimark channel.",
        "The project is approved by Smart Steel.",
        "The customer accepts the approved Agrimark customer proposal.",
        "Any required payment condition, deposit or finance approval has been met.",
        "Agrimark issues the required purchase order or formal instruction to Smart Steel.",
...
def page_marketing(c, page_no):
    y = start_page(c, "08", "Agrimark-Led Marketing Approach", "Agrimark should control the customer-facing marketing campaign for the pilot, with Smart Steel available to support where requested.", page_no)
    y = cards3(c, y - 4, [
        ("Lead", "Partner-led marketing", "Agrimark drives campaign timing, channels, message and customer activation through its own market reach."),
        ("Support", "Smart Steel on request", "Smart Steel can supply product data, creatives, technical summaries and visuals whenever Agrimark needs them."),
        ("Measure", "Atlas attribution", "Campaign source, branch, salesperson, lead status and conversion outcome are captured."),
    ], h=120)
    y = two_columns(c, y - 28, "Agrimark leads", [
...
def page_campaigns(c, page_no):
    y = start_page(c, "09", "Campaign Execution", "Campaign material should be practical, measurable and led by Agrimark, with Smart Steel support available where useful.", page_no)
    ...
    y = note(c, y - 8, "Marketing control", "Agrimark should have full control of the pilot marketing campaign. Smart Steel can provide product data, creatives, visuals and technical inputs at any point where Agrimark requests support.", h=72)
    cards3(c, y - 24, [
        ("Voice", "Agrimark customer lead", "Agrimark owns customer-facing campaign voice, channels and deployment."),
...
def page_technical(c, page_no):
    y = start_page(c, "10", "Technical Qualification", "Agrimark sales personnel should be supported by a guided workflow rather than asked to make structural decisions.", page_no)
    ...
    cards3(c, y - 18, [
        ...
        ("Supplier", "Dealer quotation", "Smart Steel quotation to Agrimark after technical and commercial approval."),
        ("Customer", "Agrimark proposal", "Customer-facing proposal using approved scope, pricing basis and exclusions."),
    ], h=106)
    c.showPage()


def page_installation(c, page_no):
    y = start_page(c, "11", "Installation Approach", "Agrimark's involvement should remain simple. Smart Steel can support installed projects, or the customer can self-install on a supply-only basis.", page_no)
    gap = 24
    w = (CONTENT_W - gap) / 2
...
def page_financing(c, page_no):
    y = start_page(c, "12", "Financing", "Agrimark's financing capability is a central differentiator in the proposed collaboration.", page_no)
    ...
    y = two_columns(c, y, "Eligibility", [
        ...
        "Payment timing from Agrimark to Smart Steel.",
        "Treatment of cancellations and customer default.",
    ])
    note(c, y - 16, "Payment principle", "Smart Steel should be paid by Agrimark according to agreed supplier payment milestones. Customer repayment performance under Agrimark financing should be treated separately from Smart Steel supplier payment terms.", h=60)
    c.showPage()


def page_atlas(c, page_no):
    y = start_page(c, "13", "Atlas Partner Dashboard", "The Agrimark-dedicated Atlas dashboard would provide the structured workflow for the collaboration.", page_no)
    ...
    y = two_columns(c, y, "Sales workflow", [
        "Secure Agrimark salesperson login.",
...
def page_warranty(c, page_no):
    y = start_page(c, "15", "Warranty and Claims", "Agrimark should remain the first point of contact for customer warranty claims.", page_no)
    y = flow(c, y - 4, [
        ("01", "Customer", "Customer contacts Agrimark."),
        ("02", "Log", "Agrimark logs the claim in Atlas."),
...
def page_pilot(c, page_no):
    ...
    table(c, MARGIN_X, y - 30, [260, 247], ["Activity", "Proposed Target"], [
        ("Agrimark lead acknowledgement", "1 business day"),
...
def page_governance(c, page_no):
    ...
    y = cards3(c, y - 4, [
        ("Owner", "Named leads", "Smart Steel and Agrimark each appoint commercial and operational owners."),
...
def page_alignment(c, page_no):
    ...
    table(c, MARGIN_X, y, [145, 362], ["Area", "Matter for Alignment"], [
        ...
        ("Marketing", "Agrimark-controlled campaigns, lead routing, Smart Steel support resources and reporting."),
...
def main():
    ...
    c.setTitle("Atlas x Agrimark Commercial Collaboration Framework")
    c.setAuthor("Smart Steel")
    c.setSubject("Draft commercial collaboration framework for Agrimark")
    c.setKeywords("Atlas, Smart Steel, Agrimark, commercial collaboration, pilot")
    draw_cover(c)
    pages = [
        page_important_note,
        page_exec,
