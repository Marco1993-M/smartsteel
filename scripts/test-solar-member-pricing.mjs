...
for (const width of [2.75, 5.5, 11, 16.5, 22]) {
  ...
  assert.equal(single.connections.find(item => item.code === 'SC-BRK-ARM').quantity, single.frames * 2)
...
assert.equal(priced.lineItems.find(item => item.code === 'SC-BRK-ARM').quantity, 4)
