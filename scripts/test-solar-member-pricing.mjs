...
for (const width of ATLAS_SOLAR_CARPORT_PARKING_COUNTS.map(getAtlasSolarCarportWidth)) {
  ...
  assert.equal(single.connections.find(item => item.code === 'SC-BRK-BASE').quantity, single.frames)
  assert.equal(single.connections.find(item => item.code === 'SC-BRK-ARM').quantity, single.frames)
  assert.equal(single.connections.find(item => item.code === 'SC-BRK-PUR').quantity, single.bays * 12)
  assert.equal(single.connections.find(item => item.code === 'SC-ANC').quantity, single.frames * 4)
  assert.equal(single.connections.find(item => item.code === 'SC-BLT-BASE').quantity, single.frames * 2)
  assert.equal(single.connections.find(item => item.code === 'SC-BLT-ARM-BASE').quantity, single.frames * 4)
  assert.equal(single.connections.find(item => item.code === 'SC-BLT-ARM-TOP').quantity, single.frames * 4)
  assert.equal(double.connections.find(item => item.code === 'SC-BRK-BASE').quantity, single.frames * 2)
...
assert.equal(priced.lineItems.find(item => item.code === 'SC-BRK-ARM').unitRate, 175)
