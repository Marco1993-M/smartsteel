...
ATLAS_LIGHT = ROOT / "public" / "atlas" / "atlas-logo-horizontal-light.png"
PARTNER_LOGO = ROOT / "public" / "overberg-agri-logo.png"
OUT = ROOT / "output" / "pdf" / "overberg-agri-commercial-collaboration-framework.pdf"
...
def draw_header_footer(c, page_no=None):
    ...
    c.drawString(MARGIN_X + 20, PAGE_H - 70, "September")
...
def draw_cover(c):
    ...
    set_rgb(c, CREAM)
    c.setLineWidth(1.2)
    c.line(58, PAGE_H - 62, PAGE_W - 58, PAGE_H - 62)
    c.line(58, 98, PAGE_W - 58, 98)

    c.setFont("Helvetica", 10.5)
    c.drawString(80, PAGE_H - 84, "September")
    c.drawRightString(PAGE_W - 80, PAGE_H - 84, "2026")

    c.setFont("Helvetica-Bold", 39)
    c.drawString(80, PAGE_H - 278, "Commercial")
    c.drawString(80, PAGE_H - 321, "Collaboration")
    c.drawString(80, PAGE_H - 364, "Framework")

    draw_image_contain(c, ATLAS_LIGHT, 80, PAGE_H - 454, 142, 46)
    set_rgb(c, CREAM)
    c.setFont("Helvetica", 24)
    c.drawCentredString(241, PAGE_H - 439, "x")
    c.rect(264, PAGE_H - 458, 188, 54, fill=1, stroke=0)
    draw_image_contain(c, PARTNER_LOGO, 274, PAGE_H - 451, 168, 40)

    set_rgb(c, CREAM)
    c.setFont("Helvetica", 9.5)
    c.drawString(80, 76, "A collaborative discussion paper for a controlled pilot across Atlas Warehouses,")
    c.drawString(80, 62, "Solar Ground Mounts, Solar Carports and selected agricultural structures.")
    c.setFont("Helvetica-Oblique", 9.5)
    c.drawString(80, 41, "Atlas by Smart Steel. Draft for discussion.")
    c.showPage()
...
def page_important_note(c, page_no):
    y = start_page(c, "00", "Document Status", "This paper supports commercial and operational alignment between Smart Steel and Overberg Agri.", page_no, title_size=27)
    ...
    cards3(c, y - 24, [
        ("Audience", "Overberg Agri stakeholders", "Partnership, commercial, finance and operations teams."),
...
def page_exec(c, page_no):
    ...
    y = para(c, "Smart Steel and Overberg Agri have a potential opportunity to create a practical infrastructure collaboration for agricultural, commercial and energy-related customers.", MARGIN_X, y, CONTENT_W * 0.92, size=10.0, leading=12.4)
    y = para(c, "The proposed model combines Overberg Agri's customer relationships, finance capability, branch network and market credibility with Smart Steel's modular steel infrastructure systems, structural engineering capability, manufacturing base, technical support and Atlas digital platform.", MARGIN_X, y - 10, CONTENT_W * 0.92, size=10.0, leading=12.4)
    y = para(c, "The aim is to create a simple commercial path for Overberg Agri: generate customer opportunity and additional return, without carrying unnecessary technical, installation or operational complexity.", MARGIN_X, y - 10, CONTENT_W * 0.92, size=10.0, leading=12.4)
    y = heading(c, "The intended customer experience is simple:", MARGIN_X, y - 14, size=11)
    cards3(c, y, [
        ("Overberg Agri", "Customer channel", "Primary commercial relationship, customer-facing partner and finance channel."),
...
def page_rationale(c, page_no):
    y = start_page(c, "02", "Collaboration Rationale", "The proposed collaboration is based on a clear fit between Overberg Agri's market access and Smart Steel's infrastructure capability.", page_no)
    y = two_columns(c, y, "Overberg Agri brings", [
    ...
    note(c, y - 18, "Shared opportunity", "A customer can engage through Overberg Agri, access finance, select a structure, receive Smart Steel technical support and move through a controlled process from enquiry to delivery, installation and warranty support.", h=62)
    c.showPage()


def page_model(c, page_no):
    y = start_page(c, "03", "Proposed Collaboration Model", "The preferred structure is a protected dealer-price model, with Overberg Agri remaining the customer-facing commercial party.", page_no)
    y = flow(c, y - 8, [
        ("01", "Customer", "Overberg Agri qualifies the opportunity."),
        ("02", "Atlas", "Project information is captured."),
        ("03", "Review", "Smart Steel reviews scope and assumptions."),
        ("04", "Dealer quote", "Smart Steel quotes Overberg Agri."),
        ("05", "Proposal", "Overberg Agri issues customer proposal."),
        ("06", "Finance", "Payment or finance is managed."),
        ("07", "PO", "Overberg Agri instructs Smart Steel."),
        ("08", "Delivery", "Smart Steel executes agreed scope."),
    ], cols=4)
    y = heading(c, "Why this model", MARGIN_X, y - 18)
    y = bullet_list(c, [
        "Overberg Agri remains the primary customer relationship owner.",
...
def page_customer_journey(c, page_no):
    ...
    steps = [
        ("01", "Opportunity", "Overberg Agri receives or identifies a customer opportunity."),
        ...
        ("04", "Dealer quotation", "Smart Steel issues approved quote to Overberg Agri."),
        ("05", "Customer proposal", "Overberg Agri issues proposal and manages finance."),
        ("06", "Acceptance", "Customer accepts and Overberg Agri issues instruction."),
...
def page_commercial(c, page_no):
    ...
    y = cards3(c, y - 4, [
        ...
        ("Pilot return", "5% proposed", "Overberg Agri receives a proposed return on qualifying structural supply value."),
...
def page_return(c, page_no):
    y = start_page(c, "06", "Pilot Return Basis", "The proposed starting commercial return for Overberg Agri is 5% during the pilot and initial launch period.", page_no)
    ...
    bullet_list(c, [
        "The opportunity is properly registered through the Overberg Agri channel.",
        "The project is approved by Smart Steel.",
        "The customer accepts the approved Overberg Agri customer proposal.",
        "Any required payment condition, deposit or finance approval has been met.",
        "Overberg Agri issues the required purchase order or formal instruction to Smart Steel.",
...
def page_marketing(c, page_no):
    y = start_page(c, "08", "Overberg Agri-Led Marketing Approach", "Overberg Agri should control the customer-facing marketing campaign for the pilot, with Smart Steel available to support where requested.", page_no)
    y = cards3(c, y - 4, [
        ("Lead", "Partner-led marketing", "Overberg Agri drives campaign timing, channels, message and customer activation through its own market reach."),
        ("Support", "Smart Steel on request", "Smart Steel can supply product data, creatives, technical summaries and visuals whenever Overberg Agri needs them."),
        ("Measure", "Atlas attribution", "Campaign source, branch, salesperson, lead status and conversion outcome are captured."),
    ], h=120)
    y = two_columns(c, y - 28, "Overberg Agri leads", [
...
def page_campaigns(c, page_no):
    y = start_page(c, "09", "Campaign Execution", "Campaign material should be practical, measurable and led by Overberg Agri, with Smart Steel support available where useful.", page_no)
    ...
    y = note(c, y - 8, "Marketing control", "Overberg Agri should have full control of the pilot marketing campaign. Smart Steel can provide product data, creatives, visuals and technical inputs at any point where Overberg Agri requests support.", h=72)
    cards3(c, y - 24, [
        ("Voice", "Overberg Agri customer lead", "Overberg Agri owns customer-facing campaign voice, channels and deployment."),
...
def page_technical(c, page_no):
    y = start_page(c, "10", "Technical Qualification", "Overberg Agri sales personnel should be supported by a guided workflow rather than asked to make structural decisions.", page_no)
    ...
    cards3(c, y - 18, [
        ...
        ("Supplier", "Dealer quotation", "Smart Steel quotation to Overberg Agri after technical and commercial approval."),
        ("Customer", "Overberg Agri proposal", "Customer-facing proposal using approved scope, pricing basis and exclusions."),
    ], h=106)
    c.showPage()


def page_installation(c, page_no):
    y = start_page(c, "11", "Installation Approach", "Overberg Agri's involvement should remain simple. Smart Steel can support installed projects, or the customer can self-install on a supply-only basis.", page_no)
    gap = 24
    w = (CONTENT_W - gap) / 2
...
def page_financing(c, page_no):
    y = start_page(c, "12", "Financing", "Overberg Agri's financing capability is a central differentiator in the proposed collaboration.", page_no)
    ...
    y = two_columns(c, y, "Eligibility", [
        ...
        "Payment timing from Overberg Agri to Smart Steel.",
        "Treatment of cancellations and customer default.",
    ])
    note(c, y - 16, "Payment principle", "Smart Steel should be paid by Overberg Agri according to agreed supplier payment milestones. Customer repayment performance under Overberg Agri financing should be treated separately from Smart Steel supplier payment terms.", h=60)
    c.showPage()


def page_atlas(c, page_no):
    y = start_page(c, "13", "Atlas Partner Dashboard", "The Overberg Agri-dedicated Atlas dashboard would provide the structured workflow for the collaboration.", page_no)
    ...
    y = two_columns(c, y, "Sales workflow", [
        "Secure Overberg Agri salesperson login.",
...
def page_warranty(c, page_no):
    y = start_page(c, "15", "Warranty and Claims", "Overberg Agri should remain the first point of contact for customer warranty claims.", page_no)
    y = flow(c, y - 4, [
        ("01", "Customer", "Customer contacts Overberg Agri."),
        ("02", "Log", "Overberg Agri logs the claim in Atlas."),
...
def page_pilot(c, page_no):
    ...
    table(c, MARGIN_X, y - 30, [260, 247], ["Activity", "Proposed Target"], [
        ("Overberg Agri lead acknowledgement", "1 business day"),
...
def page_governance(c, page_no):
    ...
    y = cards3(c, y - 4, [
        ("Owner", "Named leads", "Smart Steel and Overberg Agri each appoint commercial and operational owners."),
...
def page_alignment(c, page_no):
    ...
    table(c, MARGIN_X, y, [145, 362], ["Area", "Matter for Alignment"], [
        ...
        ("Marketing", "Overberg Agri-controlled campaigns, lead routing, Smart Steel support resources and reporting."),
...
def main():
    ...
    c.setTitle("Atlas x Overberg Agri Commercial Collaboration Framework")
    c.setAuthor("Smart Steel")
    c.setSubject("Draft commercial collaboration framework for Overberg Agri")
    c.setKeywords("Atlas, Smart Steel, Overberg Agri, commercial collaboration, pilot")
    draw_cover(c)
    pages = [
        page_important_note,
        page_exec,
