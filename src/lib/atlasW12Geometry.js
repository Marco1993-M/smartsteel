...
export const ATLAS_W12_LENGTHS_M = [4, 8, 12, 16, 20, 24, 28, 32, 36, 40, 44, 48, 52, 56, 60]
export const ATLAS_W12_EAVE_HEIGHTS_M = [3, 4, 4.5, 5]

export const ATLAS_W12_PROFILES = {
...
const PROFILE = ATLAS_W12_PROFILES
...
export function calculateAtlasW12Geometry({ lengthM = 20, eaveHeightM = 4.5 } = {}) {
  ...
  if (!ATLAS_W12_LENGTHS_M.includes(length)) throw new Error("W12 length must use 4m bays from 4m to 60m.")
  ...
  const frontGableColumnLengthM = 5.3 + gableHeightAdjustmentM
  ...
  const members = {
    ...
    wallBracing: { code: "W12-XBW", label: "Wall X-bracing", quantity: bracedBayPositions.length * 4, cutLengthM: wallBraceCutLengthM, massKgPerM: calculateLippedChannelMassKgPerM(PROFILE.bracing), rule: `${bracedBayPositions.length} braced bays × 2 walls × 2 diagonals` },
    roofBracing: { code: "W12-XBR", label: "Roof X-bracing", quantity: bracedBayPositions.length * 4, cutLengthM: roofBraceCutLengthM, massKgPerM: calculateLippedChannelMassKgPerM(PROFILE.bracing), rule: `${bracedBayPositions.length} braced bays × 2 roof slopes × 2 diagonals` },
    sideGirts: { code: "W12-GRT", label: "Side girts", quantity: bays * 2 * 3, cutLengthM: baySpacingM, massKgPerM: calculateLippedChannelMassKgPerM(PROFILE.sideGirt), rule: `${bays} bays × 2 walls × 3 rows` },
    frontGableColumns: { code: "W12-GCF", label: "Front gable columns", quantity: 2, cutLengthM: frontGableColumnLengthM, massKgPerM: calculateLippedChannelMassKgPerM(PROFILE.gableColumn), rule: "2 single columns at 3m / 6m / 3m spacing" },
    rearGableColumns: { code: "W12-GCR", label: "Rear gable columns", quantity: 2, cutLengthM: rearGableColumnLengthM, massKgPerM: calculateLippedChannelMassKgPerM(PROFILE.gableColumn), rule: "2 single columns at 4m / 4m / 4m spacing" },
    apexMembers: { code: "W12-APX", label: "Apex members", quantity: portalFrames, cutLengthM: apexMemberLengthM, massKgPerM: calculateLippedChannelMassKgPerM(PROFILE.apexHaunch), rule: `${portalFrames} frames × 1 apex member` },
    haunchMembers: { code: "W12-HCH", label: "Haunch members", quantity: portalFrames * 2, cutLengthM: haunchMemberLengthM, massKgPerM: calculateLippedChannelMassKgPerM(PROFILE.apexHaunch), rule: `${portalFrames} frames × 2 haunch members` },
  }
...
export function applyAtlasW12GeometryToPricing(records, geometry) {
  ...
  return records.map((record) => {
    if (record.componentCode === "W12-APH") {
