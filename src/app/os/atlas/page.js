      ...
      <section className="grid gap-4 xl:grid-cols-[minmax(0,1.1fr)_minmax(0,0.9fr)]">
        <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
          ...
          <h3 className="mt-2 text-3xl font-bold tracking-tight text-slate-900">
            Atlas product workspace
          </h3>
          <p className="mt-3 max-w-3xl text-sm leading-6 text-slate-600">
            The working home for the lip channel line: products, components, pricing, engineering references, and documents in one place.
          </p>
          <div className="mt-5 flex flex-wrap gap-3">
            <Link
              href="/os/atlas/products"
        ...
        <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <h3 className="mt-1 text-2xl font-bold tracking-tight text-slate-900">
              Structured around the Atlas line, not loose quote naming
            </h3>
          </div>
          ...
          <div className="mt-4 grid gap-3 md:grid-cols-2">
            {ATLAS_MODULES.map((module) => (
              <div key={module.key} className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                ...
                {ATLAS_NAV_ITEMS.find((item) => item.label === module.label) ? (
                  <Link
                    href={ATLAS_NAV_ITEMS.find((item) => item.label === module.label).href}
