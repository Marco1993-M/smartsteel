...
import { ATLAS_SOLAR_CARPORT_PARKING_COUNTS, getAtlasSolarCarportWidth } from 'lib/atlasSolarCarportLayouts'
...
export default function SolarPricingPage() {
  ...
  return <main className="space-y-6 px-3 py-4 text-slate-900 sm:px-6 sm:py-6">
    ...
    {costs && <>
      ...
      <section id="schedule" className="scroll-mt-6 border border-slate-200 bg-white shadow-sm"><div className="flex flex-wrap items-end justify-between gap-4 border-b border-slate-200 p-5"><div><p className="text-xs font-black uppercase tracking-[0.16em] text-blue-700">Configuration check</p><h2 className="mt-1 text-2xl font-bold">Material and connection schedule</h2></div><div className="flex gap-3"><label className="text-xs font-bold">Parking width<select className="mt-1 block border border-slate-300 p-2" value={width} onChange={e => setWidth(Number(e.target.value))}>{ATLAS_SOLAR_CARPORT_PARKING_COUNTS.map(getAtlasSolarCarportWidth).map(w => <option key={w} value={w}>{w}m</option>)}</select></label><label className="text-xs font-bold">Layout<select className="mt-1 block border border-slate-300 p-2" value={length} onChange={e => setLength(Number(e.target.value))}><option value={6}>Single row</option><option value={12}>Double row</option></select></label></div></div>
