      ...
      <section className="mx-auto flex min-h-[calc(100vh-4rem)] w-full max-w-lg items-center lg:min-h-screen lg:px-12">
        <div className="w-full rounded-[2rem] border border-slate-200 bg-white p-7 shadow-xl shadow-slate-900/5 sm:p-10">
          <div className="flex w-full items-center justify-between gap-4 overflow-hidden lg:hidden">
            ...
            <Image src={portal.logo} alt={`${portal.name}`} width={105} height={40} className="h-auto w-[24%] max-w-[5.5rem] shrink-0 object-contain object-right" priority />
          </div>
          ...
          <p className="mt-3 text-sm leading-6 text-slate-600">Sign in with your approved {portal.name} portal account.</p>
          ...
          <p className="mt-6 text-center text-xs leading-5 text-slate-400">Need access? Ask your {portal.name} portal administrator.</p>
        </div>
      </section>
    </main>
  )
