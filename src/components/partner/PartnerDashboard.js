      ...
      <div className="mx-auto max-w-[1500px] px-4 py-5 sm:px-6 sm:py-8 lg:px-10">
        ...
        <section className="mt-7 grid gap-5 lg:grid-cols-[1.15fr_0.85fr]">
          <div className="rounded-[1.75rem] border border-slate-200 bg-white p-5 sm:p-7">
            ...
            {primaryProduct ? <div className="mt-6 rounded-2xl bg-[#eef4f8] p-5"><p className="text-sm font-bold text-slate-500">{primaryProduct.productCode}</p><h3 className="mt-1 text-2xl font-black">{primaryProduct.name}</h3><div className="mt-5 grid grid-cols-2 gap-4 text-sm sm:grid-cols-4"><div><p className="text-slate-500">Size</p><p className="mt-1 font-black">{primaryProduct.summary.size}</p></div><div><p className="text-slate-500">Finish</p><p className="mt-1 font-black">{primaryProduct.summary.steelFinish}</p></div><div><p className="text-slate-500">Scope</p><p className="mt-1 font-black">{primaryProduct.summary.sheeting}</p></div><div><p className="text-slate-500">Guide excl. VAT</p><p className="mt-1 font-black text-[#0043f3]">{currency.format(primaryProduct.price?.amount_ex_vat || 0)}</p></div></div><button type="button" onClick={() => setFormOpen(true)} className="mt-5 inline-flex min-h-11 items-center gap-2 rounded-xl bg-[#0043f3] px-5 text-sm font-black text-white">Use this configuration <ArrowRight className="h-4 w-4" /></button></div> : <p className="mt-6 rounded-2xl bg-amber-50 p-5 text-sm text-amber-800">No approved product release is available.</p>}
          </div>
...
function QueueCard({ id, title, empty, records, icon: Icon, onOpen }) {
  return <section id={id} className="scroll-mt-5 rounded-[1.75rem] border border-slate-200 bg-white p-5 sm:p-6"><div className="flex items-center gap-3"><span className="grid h-10 w-10 place-items-center rounded-xl bg-[#c1d9e5] text-[#0043f3]"><Icon className="h-5 w-5" /></span><h2 className="text-lg font-black">{title}</h2></div>{records.length ? <div className="mt-4 space-y-2">{records.slice(0, 4).map((record) => { const Tag = onOpen ? "button" : "div"; return <Tag key={record.id} type={onOpen ? "button" : undefined} onClick={onOpen ? () => onOpen(record) : undefined} className="block w-full rounded-xl border border-slate-200 px-4 py-3 text-left transition hover:border-blue-300"><div className="flex items-center justify-between gap-3"><p className="font-bold">{record.customer_name}</p><span className="text-[10px] font-bold uppercase tracking-[0.12em] text-[#0043f3]">{record.status.replaceAll("_", " ")}</span></div><p className="mt-1 text-xs text-slate-500">{record.reference} · {record.configuration?.width}m × {record.configuration?.length}m</p>{record.status === "quoted" && record.quote_url ? <div className="mt-3 flex items-center justify-between gap-3 border-t border-slate-200 pt-3"><div><p className="text-[9px] font-bold uppercase tracking-[0.14em] text-slate-400">Approved quote excl. VAT</p><p className="mt-1 font-black text-[#0043f3]">{currency.format(record.final_quote_amount_ex_vat || 0)}</p></div><a href={record.quote_url} target="_blank" rel="noreferrer" className="rounded-lg bg-[#0043f3] px-3 py-2 text-xs font-black text-white">Open quote</a></div> : null}{record.partner_quote_message ? <p className="mt-2 text-xs leading-5 text-slate-600">{record.partner_quote_message}</p> : null}</Tag> })}</div> : <p className="mt-5 text-sm text-slate-500">{empty}</p>}</section>
}
