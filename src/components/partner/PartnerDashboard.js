...
function temporaryReference(record) {
  return `TEMP-${String(record.reference || "Partner").toUpperCase()}`
}
      ...
      <header className="border-b border-slate-200 bg-white px-4 py-3 sm:px-6 lg:px-10">
        <div className="mx-auto flex max-w-[1500px] items-center justify-between gap-4">
          <div className="flex min-w-0 items-center gap-4">
            ...
            <Image src={portal.logo} alt={`${portal.name}`} width={105} height={38} className="hidden h-8 w-auto object-contain sm:block" priority />
          </div>
...
export function PartnerOrderHandoff({ record, onSubmitted }) {
  ...
  async function submitOrder() {
    if (!reference.trim()) return setError(`Add the ${portal.name} order or reference number.`)
    setSaving(true)
    setError("")
    try {
      const response = await fetch("/api/partner/opportunities", {
      ...
      const payload = await response.json()
      if (!response.ok) throw new Error(payload.error || `The ${portal.name} order could not be submitted.`)
      await onSubmitted?.()
    } catch (submitError) {
  ...
  if (isExpiredPrice(record.price_valid_until)) return <div className="mt-3 rounded-xl border border-amber-200 bg-amber-50 p-4"><div className="flex items-center gap-2 text-amber-800"><Clock3 className="h-4 w-4" /><p className="text-xs font-black uppercase tracking-[0.12em]">Price review required</p></div><p className="mt-2 text-xs leading-5 text-amber-900">This approved price expired on {formatDate(record.price_valid_until)}. Smart Steel must review and reissue it before the opportunity can proceed.</p><textarea value={responseNote} onChange={(event) => setResponseNote(event.target.value)} rows="2" placeholder="Optional note for Smart Steel" className="mt-3 w-full rounded-xl border border-amber-200 bg-white p-3 text-base outline-none focus:border-amber-500" /><button type="button" disabled={saving} onClick={() => updateHandoff("request_clarification", { note: responseNote.trim() || "Please review and reissue the expired approved price." })} className="mt-2 inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-xl bg-amber-600 px-3 text-xs font-black text-white disabled:opacity-60"><MessageSquareText className="h-4 w-4" />Request updated price</button>{error ? <p className="mt-2 text-xs font-bold text-rose-700">{error}</p> : null}</div>
