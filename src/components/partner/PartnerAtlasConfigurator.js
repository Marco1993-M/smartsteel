...
export default function PartnerAtlasConfigurator({ product, initialOpportunity = null, onClose, onCreated }) {
  ...
  const [customer, setCustomer] = useState({
    ...
    installationScope: initialOpportunity?.installation_scope || {
      requested: false,
      foundation: false,
      structure: false,
      sheeting: false,
  ...
  async function saveOpportunity(submit) {
    ...
    if (customer.installationScope.requested && !customer.siteLocation.trim()) {
      return setError("Add the project location for an installation review.")
    }
    if (customer.installationScope.requested && !customer.installationScope.foundation && !customer.installationScope.structure && !customer.installationScope.sheeting) {
      return setError("Choose at least one installation scope for Smart Steel to review.")
    }
    setSaving(true)
    setError("")
    try {
        ...
        <div className="grid gap-5 xl:grid-cols-[minmax(0,1.35fr)_440px] xl:items-start">
          <section className="xl:sticky xl:top-20">
            <div className="rounded-[1.75rem] border border-slate-200 bg-white p-3 shadow-sm sm:p-4">
              ...
              <div className="mt-3 flex flex-wrap items-center justify-between gap-3 rounded-2xl bg-[#001d2e] px-4 py-3 text-white sm:px-5"><div><p className="text-[9px] font-bold uppercase tracking-[0.17em] text-[#c1d9e5]">Indicative price excl. VAT</p><p className="mt-1 text-2xl font-black">{previewing ? "Updating..." : preview ? money.format(preview.commercial.amountExVat) : "Review required"}</p>{preview?.commercial?.partnerAdjustmentRate ? <p className="mt-1 text-[10px] font-semibold text-white/55">Includes the agreed {Math.round(preview.commercial.partnerAdjustmentRate * 100)}% partner adjustment</p> : null}</div><p className="text-sm font-bold text-white/70">{configuration.width}m × {configuration.length}m × {configuration.wallHeight}m</p></div>
            </div>
          </section>
...
function CustomerStep({ customer, setCustomer, preview }) {
  ...
  const setInstallationScope = (updates) => setCustomer((current) => ({
    ...current,
    installationScope: { ...current.installationScope, ...updates },
  }))
  return <div><StepHeading number="3" title="Review and add the customer" />{preview?.lineItem ? <div className="mt-5 overflow-hidden rounded-2xl border border-blue-200 bg-[#eef4f8]"><div className="flex items-center gap-3 border-b border-blue-100 px-4 py-3"><span className="grid h-9 w-9 place-items-center rounded-xl bg-white text-[#0043f3]"><PackageCheck className="h-4 w-4" /></span><div className="min-w-0"><p className="text-[9px] font-black uppercase tracking-[0.16em] text-slate-400">Atlas configuration</p><p className="truncate font-mono text-xs font-black text-[#0043f3]">{preview.sku}</p></div></div><div className="px-4 py-4"><p className="text-sm font-bold leading-6 text-[#001d2e]">{preview.lineItem.description}</p><div className="mt-3 flex items-end justify-between gap-3 border-t border-blue-100 pt-3"><div><p className="text-[9px] font-black uppercase tracking-[0.14em] text-slate-400">Quantity</p><p className="mt-1 text-sm font-black">1 structure</p></div><div className="text-right"><p className="text-[9px] font-black uppercase tracking-[0.14em] text-slate-400">Indicative price excl. VAT</p><p className="mt-1 text-lg font-black text-[#0043f3]">{money.format(preview.commercial.amountExVat)}</p></div></div><p className="mt-3 border-t border-blue-100 pt-3 text-xs leading-5 text-slate-500">Based on the current approved Atlas pricing release. Final pricing is confirmed by Smart Steel after project review.</p></div></div> : null}<p className="mt-5 text-sm leading-6 text-slate-500">Only the customer name is required to save a draft.</p><div className="mt-5 space-y-4"><Field label="Customer name" required><input {...field("customerName")} /></Field><div className="grid gap-4 sm:grid-cols-2"><Field label="Mobile"><input {...field("customerPhone")} inputMode="tel" /></Field><Field label="Email"><input {...field("customerEmail")} type="email" /></Field></div><Field label={`Project location${customer.installationScope.requested ? " *" : ""}`}><input {...field("siteLocation")} placeholder="Town, farm or site address" /></Field><section className="rounded-2xl border border-slate-200 p-4"><label className="flex cursor-pointer items-start gap-3"><input type="checkbox" checked={customer.installationScope.requested} onChange={(event) => setInstallationScope(event.target.checked ? { requested: true } : { requested: false, foundation: false, structure: false, sheeting: false })} className="mt-0.5 h-5 w-5 accent-[#0043f3]" /><span><span className="block text-sm font-black text-[#001d2e]">Request an installation review</span><span className="mt-1 block text-xs leading-5 text-slate-500">Smart Steel will review the site and confirm installation scope and pricing separately.</span></span></label>{customer.installationScope.requested ? <div className="mt-4 grid gap-2 border-t border-slate-200 pt-4 sm:grid-cols-3">{[["foundation", "Foundation"], ["structure", "Structure"], ["sheeting", "Sheeting"]].map(([key, label]) => <label key={key} className={`flex min-h-11 cursor-pointer items-center gap-2 rounded-xl border px-3 text-xs font-black ${customer.installationScope[key] ? "border-[#0043f3] bg-blue-50 text-[#0043f3]" : "border-slate-200 text-slate-600"}`}><input type="checkbox" checked={customer.installationScope[key]} onChange={(event) => setInstallationScope({ [key]: event.target.checked })} className="h-4 w-4 accent-[#0043f3]" />{label}</label>)}</div> : null}{customer.installationScope.requested ? <p className="mt-3 text-[11px] leading-5 text-slate-500">Site clearance, earthworks and services are not included in this request.</p> : null}</section><Field label="Useful note"><textarea {...field("notes")} rows="3" placeholder="What will the building be used for?" /></Field></div></div>
}
