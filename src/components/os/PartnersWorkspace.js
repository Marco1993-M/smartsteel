    ...
    <div className="min-w-0">
      <div className="sticky top-0 z-20 border-b border-slate-200 bg-white/95 px-4 py-3 backdrop-blur sm:px-6">
        <div className="flex gap-2 overflow-x-auto">
          {[{ value: "opportunities", label: "Opportunities" }, { value: "rolodex", label: "Rolodex" }, { value: "releases", label: "Commercial releases" }, { value: "network", label: "AFGRI network" }].map((option) => (
      ...
      {view === "opportunities" ? <PartnerOpportunityWorkspace /> : view === "rolodex" ? <PartnerRolodexWorkspace /> : view === "network" ? <PartnerBranchesWorkspace /> : <PartnerReleaseWorkspace />}
    </div>
  )
}
