...
export default function ManufacturingWorkspace() {
  ...
  async function adoptBom(record) {
    setSavingId(record.id)
    setError("")
    try {
      const response = await fetch("/api/os/partner-opportunities", {
        method: "PATCH",
        headers: await getOsAuthHeaders({ "Content-Type": "application/json" }),
        body: JSON.stringify({ id: record.id, action: "adopt_bom_snapshot", adoptedBy: "Smart Steel production team" }),
      })
      const payload = await response.json()
      if (!response.ok) throw new Error(payload.error || "Could not adopt the approved BOM.")
      setRecords((current) => current.map((item) => item.id === payload.record.id ? payload.record : item))
    } catch (saveError) {
      setError(saveError.message)
    } finally {
  ...
  return <div className="min-w-0 space-y-5 px-4 py-5 sm:px-6 sm:py-6">
    ...
    <section className="rounded-[24px] border border-slate-200 bg-white p-4 shadow-sm sm:p-6">
      ...
      <div className="mt-5 space-y-3">{loading ? <p className="py-12 text-center text-sm text-slate-500">Loading production orders...</p> : shown.length ? shown.map((record) => <ProductionCard key={record.id} record={record} saving={savingId === record.id} onMove={moveOrder} onUpdatePlan={updatePlan} onUpdateRelease={updateRelease} onAdoptBom={adoptBom} onOpenPack={setPackRecord} />) : <div className="grid min-h-52 place-items-center rounded-2xl border border-dashed border-slate-300 bg-slate-50 text-center"><div><PackageCheck className="mx-auto h-8 w-8 text-slate-300" /><p className="mt-3 font-bold text-slate-600">No orders in this stage.</p></div></div>}</div>
    </section>
  </div>
}
...
function ProductionCard({ record, saving, onMove, onUpdatePlan, onUpdateRelease, onAdoptBom, onOpenPack }) {
  ...
  return <article className="rounded-2xl border border-slate-200 p-4 sm:p-5"><div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-start"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><span className="rounded-full bg-blue-100 px-2.5 py-1 text-[9px] font-black uppercase tracking-[0.13em] text-blue-800">AFGRI partner</span><span className="rounded-full bg-emerald-100 px-2.5 py-1 text-[9px] font-black uppercase tracking-[0.13em] text-emerald-800">{stageLabel(status)}</span></div><h3 className="mt-3 truncate text-xl font-black text-slate-950">{record.customerName}</h3><p className="mt-1 font-mono text-xs font-bold text-[#0043f3]">{record.afgriOrderReference || record.reference}</p></div><div className="grid grid-cols-2 gap-x-6 gap-y-2 text-sm sm:grid-cols-3"><Detail label="Configuration" value={`${config.width}m × ${config.length}m × ${config.wallHeight}m`} /><Detail label="Dispatch" value={formatDate(record.estimatedDispatchDate)} /><Detail label="Delivery" value={formatDate(record.estimatedDeliveryDate)} /></div></div>
    ...
    <div className={`mt-3 border p-4 ${record.manufacturingBomState?.status === "current" ? "border-sky-300 bg-sky-50" : record.manufacturingBomState?.status === "update_available" ? "border-amber-300 bg-amber-50" : "border-slate-300 bg-slate-50"}`}>
      <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-center"><div><p className="text-[10px] font-black uppercase tracking-[0.15em] text-slate-500">Controlled BOM</p><p className="mt-1 text-sm font-black text-slate-950">{record.manufacturingBomState?.status === "current" ? `${record.manufacturingBomSnapshot?.source?.code} ${record.manufacturingBomSnapshot?.source?.revision} adopted` : record.manufacturingBomState?.status === "update_available" ? `${record.manufacturingBomState.latestRevision} is ready to adopt` : record.manufacturingBomState?.status === "missing" ? "No approved BOM available" : "Approved BOM not yet adopted"}</p><p className="mt-1 text-xs text-slate-600">{record.manufacturingBomState?.status === "current" ? `${record.manufacturingBomSnapshot?.materialSchedule?.length || 0} configured material lines frozen for this order.` : record.manufacturingBomState?.status === "update_available" ? "Review and adopt the newer revision. An existing production release will be revoked safely." : "Adopt an approved revision before releasing production."}</p></div>{record.manufacturingBomState?.status !== "missing" && record.manufacturingBomState?.status !== "current" ? <button type="button" disabled={saving} onClick={() => onAdoptBom(record)} className="min-h-10 bg-[#001d2e] px-4 text-xs font-black text-white disabled:opacity-40">{record.manufacturingBomState?.status === "update_available" ? "Adopt latest revision" : "Prepare manufacturing BOM"}</button> : null}</div>
    </div>
...
function getReadiness(record, plan) {
  ...
  if (record.manufacturingBomState?.status !== "current") missing.push(record.manufacturingBomState?.status === "update_available" ? "Adopt latest BOM" : "Adopt approved BOM")
...
function ManufacturingPack({ record, onClose }) {
  ...
  return <div className="fixed inset-0 z-[100] overflow-y-auto bg-slate-950/75 p-3 backdrop-blur-sm print:static print:bg-white print:p-0">
    ...
    <article className="mx-auto min-h-[297mm] w-full max-w-[210mm] bg-white p-7 text-slate-950 shadow-2xl print:min-h-0 print:max-w-none print:p-[14mm] print:shadow-none sm:p-10">
      ...
      <section className="mt-8 break-before-page print:break-before-page"><div className="flex items-end justify-between gap-4 border-b border-slate-300 pb-3"><div><p className="text-[10px] font-black uppercase tracking-[0.18em] text-slate-400">Controlled material schedule</p><h2 className="mt-1 text-xl font-black">{bom ? `${bom.source.code} · ${bom.source.revision}` : "Approved BOM required for issue"}</h2></div>{bom ? <p className="text-xs font-bold text-slate-500">{bom.materialSchedule.length} lines · {Number(bom.totalSteelKg || 0).toLocaleString("en-ZA")}kg steel</p> : null}</div>{bom ? <div className="mt-3 overflow-hidden border border-slate-300"><table className="w-full border-collapse text-left text-[10px]"><thead className="bg-[#001d2e] text-white"><tr><th className="p-2">Code</th><th className="p-2">Component</th><th className="p-2 text-right">Qty</th><th className="p-2">Unit / cut</th><th className="p-2 text-right">Mass</th></tr></thead><tbody className="divide-y divide-slate-200">{bom.materialSchedule.map((line) => <tr key={`${line.code}-${line.lineNumber}`} className={line.provisional ? "bg-amber-50" : "bg-white"}><td className="p-2 font-mono font-bold text-[#0043f3]">{line.code}</td><td className="p-2"><p className="font-bold text-slate-900">{line.description}</p><p className="mt-0.5 text-[9px] text-slate-500">{line.quantityRule}</p></td><td className="p-2 text-right font-bold">{Number(line.quantity).toLocaleString("en-ZA")}</td><td className="p-2">{line.cutLengthM ? `${line.cutLengthM}m each` : line.unit}</td><td className="p-2 text-right">{line.totalMassKg ? `${Number(line.totalMassKg).toLocaleString("en-ZA")}kg` : "-"}</td></tr>)}</tbody></table></div> : <p className="mt-3 border border-amber-300 bg-amber-50 p-4 text-sm font-bold text-amber-900">Adopt the approved Atlas BOM before this pack is released.</p>}{bom?.provisionalItems?.length ? <div className="mt-4 border border-amber-300 bg-amber-50 p-4"><p className="text-[10px] font-black uppercase tracking-[0.14em] text-amber-800">Controlled holds</p><p className="mt-2 text-xs leading-5 text-amber-950">{bom.provisionalItems.join(" · ")}</p></div> : null}</section>
      ...
      <footer className="mt-10 grid gap-4 border-t border-slate-300 pt-5 text-xs text-slate-500 sm:grid-cols-2"><div><p className="font-black uppercase tracking-[0.12em] text-slate-800">Release record</p><p className="mt-2">{released ? `${record.productionReleasedBy} · ${formatDateTime(record.productionReleasedAt)}` : "Release authority not yet recorded."}</p>{bom ? <p className="mt-1">BOM adopted by {record.manufacturingBomAdoptedBy} · {formatDateTime(record.manufacturingBomAdoptedAt)}</p> : null}</div><div className="sm:text-right"><p className="font-black uppercase tracking-[0.12em] text-slate-800">Source references</p><p className="mt-2">Partner: {record.reference}<br />Order: {record.afgriOrderReference || "Temporary reference"}<br />BOM: {bom ? `${bom.source.code} ${bom.source.revision}` : "Not adopted"}</p></div></footer>
    </article>
  </div>
}
