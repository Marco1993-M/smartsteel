...
function OpportunityRow({ record, onOpen }) {
  ...
  return <button type="button" onClick={onOpen} className="grid w-full gap-4 rounded-2xl border border-slate-200 p-4 text-left transition hover:border-[#0043f3] hover:bg-blue-50/30 sm:grid-cols-[minmax(0,1.2fr)_minmax(0,1fr)_auto] sm:items-center"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><p className="truncate font-black text-slate-950">{record.customerName}</p><span className={`rounded-full px-2.5 py-1 text-[9px] font-bold uppercase tracking-[0.12em] ${meta.className}`}>{meta.label}</span></div><p className="mt-1 truncate text-xs text-slate-500">{record.reference} · {record.partner?.name || "AFGRI"}</p></div><div><p className="font-bold text-slate-900">W{String(config.width || "").padStart(2, "0")} · {config.width}m × {config.length}m × {config.wallHeight}m</p><p className="mt-1 text-xs text-slate-500">{config.steelFinish} · {config.gableMode === "structure_only" ? "Structure only" : "Sheeting selected"}</p></div><div className="flex items-center justify-between gap-4 sm:justify-end"><p className="font-black text-[#0043f3]">{money.format(record.indicativeAmountExVat)}</p><ArrowRight className="h-4 w-4 text-slate-400" /></div></button>
}
