    ...
    <section className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        ...
        <div className="flex items-center gap-2">
          <button type="button" onClick={() => setWeekStart((current) => addDays(current, -7))} className="grid h-9 w-9 place-items-center rounded-full border border-slate-300 text-lg font-semibold text-slate-700 transition hover:bg-slate-50" aria-label="Previous week">&larr;</button>
          <button type="button" onClick={() => setWeekStart(startOfWeek(new Date()))} className="rounded-full border border-slate-300 px-3 py-2 text-xs font-semibold text-slate-700 transition hover:bg-slate-50">This week</button>
          <button type="button" onClick={() => setWeekStart((current) => addDays(current, 7))} className="grid h-9 w-9 place-items-center rounded-full border border-slate-300 text-lg font-semibold text-slate-700 transition hover:bg-slate-50" aria-label="Next week">&rarr;</button>
        </div>
      </div>
            ...
            <div key={dateKey} className={`min-h-[184px] shrink-0 border transition lg:min-h-[210px] lg:w-auto lg:p-4 ${isMobileFocus ? "w-[144px] p-2.5" : "w-[38px] p-1"} ${isToday ? "border-sky-300 bg-sky-50/60" : "border-slate-200 bg-slate-50/60"}`}>
              ...
              {isActive ? (
                <form onSubmit={addNote} className="mt-3 border-t border-slate-200 pt-3">
                  ...
                  <input id={`schedule-note-${dateKey}`} autoFocus value={note} onChange={(event) => setNote(event.target.value)} className="w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm outline-none transition focus:border-slate-500" placeholder="Call Tommy" />
